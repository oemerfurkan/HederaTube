...
const voucherSignerId =
  process.env.HEDERA_VOUCHER_SIGNER_ACCOUNT_ID?.trim() || undefined;
const voucherSignerKey =
  process.env.HEDERA_VOUCHER_SIGNER_PRIVATE_KEY?.trim() || undefined;
...
async function main(): Promise<void> {
  ...
  console.log(
    `payer: ${signer.accountId} (${signer.evmAddress}, ${signer.keyType})`,
  );
  console.log(
    `payerAuthorizer: ${voucherSigner?.evmAddress ?? signer.evmAddress}\n`,
  );
  ...
  if (refundAfterRequests) {
    ...
    console.log(
      `Refund completed in ${((performance.now() - t0) / 1000).toFixed(3)}s`,
    );
  }
}
