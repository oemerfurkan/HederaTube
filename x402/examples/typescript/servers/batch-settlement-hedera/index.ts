...
const authorizerKey =
  process.env.HEDERA_RECEIVER_AUTHORIZER_PRIVATE_KEY?.trim();
const receiverAuthorizerSigner =
  authorizerId && authorizerKey
    ? await createHederaAuthorizerSigner(
        authorizerId,
...
const channelManager = batchedScheme.createChannelManager(
  facilitatorClient,
  NETWORK,
);
...
channelManager.start({
  ...
  onRefund: (r) =>
    console.log(`Refunded channel ${r.channel} (tx: ${r.transaction})`),
...
async function main() {
  ...
  app.listen(4021, () => {
    console.log(
      "Hedera batch-settlement server listening at http://localhost:4021",
    );
